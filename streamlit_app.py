...
def main():
    ...
    st.write("""
    ## 주가 및 거래량 차트
    1. 사이드바에서 S&P 500 주식, 시작일, 종료일을 선택하세요.
    2. 원하는 기술적 지표(거래량, SMA, 볼린저 밴드, RSI)를 선택하고 매개변수를 조정하세요.
    - SMA : 이동평균선으로, 20일이 기본으로 설정되어 있습니다.
    - 볼린저밴드 : 2표준편차(95%신뢰도)가 기본으로 설정되어 있습니다.
    3. RSI : BB upper(70)이상에서 하락시 과열 (매도신호), 30이하 시 과매도 상태입니다.
    4. 차트를 클릭하여 자세히 보고싶은 구간을 선택하세요.
    """)
    # 사이드바 설정
    st.sidebar.header("주식 매개변수")
    available_tickers, tickers_companies_dict = get_sp500_components()
    ticker = st.sidebar.selectbox("티커", available_tickers, format_func=tickers_companies_dict.get, value="Nvidia")
    ...
    st.write("""
    - 'ARIMA 분해 수행' 버튼을 클릭하여 7일, 30일 예상 주가를 확인하세요 
    - ARIMA 모델은 과거 데이터를 바탕으로 미래 가격을 예측합니다. 
    - 단, 이는 참고용이며 실제 투자 결정에는 다양한 요소를 고려해야 합니다.
    """)
    ...
    st.header("시계열 분해 분석")
    st.write("""
    - '시계열 분해 수행' 버튼을 클릭하여 상세한 시계열 분석 결과를 확인하세요.
        - 계절성과 추세를 확인합니다. 계절성/추세를 제외한 잔차에 따른 변동성도 확인합니다.
